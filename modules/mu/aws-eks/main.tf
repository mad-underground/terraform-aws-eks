module "aws_eks" {
  ...
  attach_cluster_encryption_policy = try(local.eks_cluster.attach_cluster_encryption_policy, true)

  # "List of account maps to add to the aws-auth configmap"
  aws_auth_accounts = try(local.eks_cluster.aws_auth_accounts, [])

  # "List of Fargate profile pod execution role ARNs to add to the aws-auth configmap"
  aws_auth_fargate_profile_pod_execution_role_arns = try(local.eks_cluster.aws_auth_fargate_profile_pod_execution_role_arns, [])

  # "List of non-Windows based node IAM role ARNs to add to the aws-auth configmap"
  aws_auth_node_iam_role_arns_non_windows = try(local.eks_cluster.aws_auth_node_iam_role_arns_non_windows, [])

  # "List of Windows based node IAM role ARNs to add to the aws-auth configmap"
  aws_auth_node_iam_role_arns_windows = try(local.eks_cluster.aws_auth_node_iam_role_arns_windows, [])

  # "List of role maps to add to the aws-auth configmap"
  aws_auth_roles = try(local.eks_cluster.aws_auth_roles, [])

  # "List of user maps to add to the aws-auth configmap"
  aws_auth_users = try(local.eks_cluster.aws_auth_users, [])
  ...
  cloudwatch_log_group_kms_key_id = try(local.eks_cluster.cloudwatch_log_group_kms_key_id, null)

  # "Number of days to retain log events. Default retention - 90 days"
  cloudwatch_log_group_retention_in_days = try(local.eks_cluster.cloudwatch_log_group_retention_in_days, 90)

  # "List of additional, externally created security group IDs to attach to the cluster control plane"
  cluster_additional_security_group_ids = try(local.eks_cluster.cluster_additional_security_group_ids, [])

  # "Map of cluster addon configurations to enable for the cluster. Addon name can be the map keys or set with `name`"
  cluster_addons = try(local.eks_cluster.cluster_addons, {})

  # "Create, update, and delete timeout configurations for the cluster addons"
  cluster_addons_timeouts = try(local.eks_cluster.cluster_addons_timeouts, {})
  ...
  cluster_enabled_log_types = try(local.eks_cluster.cluster_enabled_log_types, ["audit", "api", "authenticator"])

  # "Configuration block with encryption configuration for the cluster"
  cluster_encryption_config = try(local.eks_cluster.cluster_encryption_config, {
      resources = ["secrets"]
    })

  # "Description of the cluster encryption policy created"
  cluster_encryption_policy_description = try(local.eks_cluster.cluster_encryption_policy_description, "Cluster encryption policy to allow cluster role to utilize CMK provided")

  # "Name to use on cluster encryption policy created"
  cluster_encryption_policy_name = try(local.eks_cluster.cluster_encryption_policy_name, null)

  # "Cluster encryption policy path"
  cluster_encryption_policy_path = try(local.eks_cluster.cluster_encryption_policy_path, null)

  # "A map of additional tags to add to the cluster encryption policy created"
  cluster_encryption_policy_tags = try(local.eks_cluster.cluster_encryption_policy_tags, {})

  # "Determines whether cluster encryption policy name (`cluster_encryption_policy_name`) is used as a prefix"
  cluster_encryption_policy_use_name_prefix = try(local.eks_cluster.cluster_encryption_policy_use_name_prefix, true)

  # "Indicates whether or not the Amazon EKS private API server endpoint is enabled"
  cluster_endpoint_private_access = try(local.eks_cluster.cluster_endpoint_private_access, true)

  # "Indicates whether or not the Amazon EKS public API server endpoint is enabled"
  cluster_endpoint_public_access = try(local.eks_cluster.cluster_endpoint_public_access, false)

  # "List of CIDR blocks which can access the Amazon EKS public API server endpoint"
  cluster_endpoint_public_access_cidrs = try(local.eks_cluster.cluster_endpoint_public_access_cidrs, ["0.0.0.0/0"])

  # "Base DNS domain name for the current partition (e.g., amazonaws.com in AWS Commercial, amazonaws.com.cn in AWS China)"
  cluster_iam_role_dns_suffix = try(local.eks_cluster.cluster_iam_role_dns_suffix, null)

  # "Map of cluster identity provider configurations to enable for the cluster. Note - this is different/separate from IRSA"
  cluster_identity_providers = try(local.eks_cluster.cluster_identity_providers, {})

  # "The IP family used to assign Kubernetes pod and service addresses. Valid values are `ipv4` (default) and `ipv6`. You can only specify an IP family when you create a cluster, changing this value will force a new cluster to be created"
  cluster_ip_family = try(local.eks_cluster.cluster_ip_family, null)

  # "Name of the EKS cluster"
  cluster_name = try(local.eks_cluster.cluster_name, "")
  ...
  cluster_security_group_additional_rules = try(local.eks_cluster.cluster_security_group_additional_rules, {})

  # "Description of the cluster security group created"
  cluster_security_group_description = try(local.eks_cluster.cluster_security_group_description, "EKS cluster security group")

  # "Existing security group ID to be attached to the cluster"
  cluster_security_group_id = try(local.eks_cluster.cluster_security_group_id, "")

  # "Name to use on cluster security group created"
  cluster_security_group_name = try(local.eks_cluster.cluster_security_group_name, null)

  # "A map of additional tags to add to the cluster security group created"
  cluster_security_group_tags = try(local.eks_cluster.cluster_security_group_tags, {})

  # "Determines whether cluster security group name (`cluster_security_group_name`) is used as a prefix"
  cluster_security_group_use_name_prefix = try(local.eks_cluster.cluster_security_group_use_name_prefix, true)
  ...
  cluster_service_ipv4_cidr = try(local.eks_cluster.cluster_service_ipv4_cidr, null)
  ...
  cluster_service_ipv6_cidr = try(local.eks_cluster.cluster_service_ipv6_cidr, null)

  # "A map of additional tags to add to the cluster"
  cluster_tags = try(local.eks_cluster.cluster_tags, {})

  # "Create, update, and delete timeout configurations for the cluster"
  cluster_timeouts = try(local.eks_cluster.cluster_timeouts, {})

  # "Kubernetes `<major>.<minor>` version to use for the EKS cluster (i.e.: `1.24`)"
  cluster_version = try(local.eks_cluster.cluster_version, null)
  ...
  control_plane_subnet_ids = try(local.eks_cluster.control_plane_subnet_ids, [])

  # "Controls if EKS resources should be created (affects nearly all resources)"
  create = try(local.eks_cluster.create, true)
  ...
  create_aws_auth_configmap = try(local.eks_cluster.create_aws_auth_configmap, false)
  ...
  create_cloudwatch_log_group = try(local.eks_cluster.create_cloudwatch_log_group, true)
  ...
  create_cluster_primary_security_group_tags = try(local.eks_cluster.create_cluster_primary_security_group_tags, true)
  ...
  create_cluster_security_group = try(local.eks_cluster.create_cluster_security_group, true)
  ...
  dataplane_wait_duration = try(local.eks_cluster.dataplane_wait_duration, "30s")

  # "Map of EKS managed node group default configurations"
  eks_managed_node_group_defaults = try(local.eks_cluster.eks_managed_node_group_defaults, {})

  # "Map of EKS managed node group definitions to create"
  eks_managed_node_groups = try(local.eks_cluster.eks_managed_node_groups, {})

  # "Determines whether to create an OpenID Connect Provider for EKS to enable IRSA"
  enable_irsa = try(local.eks_cluster.enable_irsa, true)

  # "Specifies whether key rotation is enabled. Defaults to `true`"
  enable_kms_key_rotation = try(local.eks_cluster.enable_kms_key_rotation, true)

  # "Map of Fargate Profile default configurations"
  fargate_profile_defaults = try(local.eks_cluster.fargate_profile_defaults, {})

  # "Map of Fargate Profile definitions to create"
  fargate_profiles = try(local.eks_cluster.fargate_profiles, {})

  # "Additional policies to be added to the IAM role"
  iam_role_additional_policies = try(local.eks_cluster.iam_role_additional_policies, {})

  # "Existing IAM role ARN for the cluster. Required if `create_iam_role` is set to `false`"
  iam_role_arn = try(local.eks_cluster.iam_role_arn, null)

  # "Description of the role"
  iam_role_description = try(local.eks_cluster.iam_role_description, null)

  # "Name to use on IAM role created"
  iam_role_name = try(local.eks_cluster.iam_role_name, null)

  # "Cluster IAM role path"
  iam_role_path = try(local.eks_cluster.iam_role_path, null)

  # "ARN of the policy that is used to set the permissions boundary for the IAM role"
  iam_role_permissions_boundary = try(local.eks_cluster.iam_role_permissions_boundary, null)

  # "A map of additional tags to add to the IAM role created"
  iam_role_tags = try(local.eks_cluster.iam_role_tags, {})

  # "Determines whether the IAM role name (`iam_role_name`) is used as a prefix"
  iam_role_use_name_prefix = try(local.eks_cluster.iam_role_use_name_prefix, true)
  ...
  kms_key_administrators = try(local.eks_cluster.kms_key_administrators, [])

  # "A list of aliases to create. Note - due to the use of `toset()`, values must be static strings and not computed values"
  kms_key_aliases = try(local.eks_cluster.kms_key_aliases, [])
  ...
  kms_key_deletion_window_in_days = try(local.eks_cluster.kms_key_deletion_window_in_days, null)

  # "The description of the key as viewed in AWS console"
  kms_key_description = try(local.eks_cluster.kms_key_description, null)

  # "Specifies whether to enable the default key policy. Defaults to `false`"
  kms_key_enable_default_policy = try(local.eks_cluster.kms_key_enable_default_policy, false)
  ...
  kms_key_override_policy_documents = try(local.eks_cluster.kms_key_override_policy_documents, [])

  # "A list of IAM ARNs for those who will have full key permissions (`kms:*`)"
  kms_key_owners = try(local.eks_cluster.kms_key_owners, [])
  ...
  kms_key_service_users = try(local.eks_cluster.kms_key_service_users, [])

  # "List of IAM policy documents that are merged together into the exported document. Statements must have unique `sid`s"
  kms_key_source_policy_documents = try(local.eks_cluster.kms_key_source_policy_documents, [])
  ...
  kms_key_users = try(local.eks_cluster.kms_key_users, [])

  # "Determines whether to manage the aws-auth configmap"
  manage_aws_auth_configmap = try(local.eks_cluster.manage_aws_auth_configmap, false)
  ...
  node_security_group_additional_rules = try(local.eks_cluster.node_security_group_additional_rules, {})

  # "Description of the node security group created"
  node_security_group_description = try(local.eks_cluster.node_security_group_description, "EKS node shared security group")
  ...
  node_security_group_enable_recommended_rules = try(local.eks_cluster.node_security_group_enable_recommended_rules, true)

  # "ID of an existing security group to attach to the node groups created"
  node_security_group_id = try(local.eks_cluster.node_security_group_id, "")

  # "Name to use on node security group created"
  node_security_group_name = try(local.eks_cluster.node_security_group_name, null)

  # "A map of additional tags to add to the node security group created"
  node_security_group_tags = try(local.eks_cluster.node_security_group_tags, {})

  # "Determines whether node security group name (`node_security_group_name`) is used as a prefix"
  node_security_group_use_name_prefix = try(local.eks_cluster.node_security_group_use_name_prefix, true)

  # "List of OpenID Connect audience client IDs to add to the IRSA provider"
  openid_connect_audiences = try(local.eks_cluster.openid_connect_audiences, [])

  # "Configuration for the AWS Outpost to provision the cluster on"
  outpost_config = try(local.eks_cluster.outpost_config, {})

  # "The separator to use between the prefix and the generated timestamp for resource names"
  prefix_separator = try(local.eks_cluster.prefix_separator, "-")
  ...
  putin_khuylo = try(local.eks_cluster.putin_khuylo, true)

  # "Map of self-managed node group default configurations"
  self_managed_node_group_defaults = try(local.eks_cluster.self_managed_node_group_defaults, {})

  # "Map of self-managed node group definitions to create"
  self_managed_node_groups = try(local.eks_cluster.self_managed_node_groups, {})
  ...
  subnet_ids = try(local.eks_cluster.subnet_ids, [])

  # "A map of tags to add to all resources"
  tags = try(local.eks_cluster.tags, {})

  # "ID of the VPC where the cluster security group will be provisioned"
  vpc_id = try(local.eks_cluster.vpc_id, null)
